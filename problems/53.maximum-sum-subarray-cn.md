...
#### 解法五 - 动态规划 ---- 这个就是著名的 Kadane Algorithm（卡丹算法）
...
所以 Kadane 算法可以浓缩成一句话：

当前累计和如果是负收益，就扔掉；如果是正收益，就继续带着走。
...
public:
    int maxSubArray(vector<int>& nums) {
        int ans = nums[0]; // 注意这里从 nums[0] 开始
        int cur = 0;       // 注意这里 从 0开始

        for (int x : nums) {
            cur = max(cur, 0) + x; // cur = max(cur + x, x); // cur = (cur > 0 ? cur : 0) + x;
            ans = max(ans, cur);
        }
