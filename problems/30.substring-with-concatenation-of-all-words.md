...
_LYJ: 滑动窗口_
https://aistudio.google.com/prompts/1mw0EUEUM_PTGPy6yQYC-DXlI50aKsUr3

[动图演示](../visual/substring-with-concatenation-of-all-words.html)
```cpp
class Solution {
public:
    vector<int> findSubstring(string s, vector<string>& words) {
        ...
        for (const string &w : words) counts[w]++; // 统计单词出现的频率

        // 关键优化：外层循环只跑 len 次
        // 因为单词长度是 len，所有的起始位置可以按照模 len 的余数分成 len 组
        for (int i = 0; i < wLen; i++) {  // 注意这里是 i < wLen ( 一个单词的长度 ) 
            int left = i, right = i, matched = 0;
            unordered_map<string_view, int> win;

            // right 每次向右移动一个单词的距离
            while (right + wLen <= sLen) {
                string_view w = s_view.substr(right, wLen);
                right += wLen;

                if (counts.count(w)) {
                    win[w]++; matched++;

                    // 如果某个单词出现的次数超过了需要的次数，开始收缩左边界
                    while (win[w] > counts[w]) { // 注意这里用了 while 只到成立。
                        string_view left_w = s_view.substr(left, wLen);
                        win[left_w]--; matched--; // 注意这里是 left_w !!
                        left += wLen;
                    }

                    // 如果窗口内的单词总数等于 words 的数量，说明匹配成功
                    if (matched == wNum) {
                        res.push_back(left);
                    }
                } else {
                    // 如果截取的单词压根不在 words 里，直接重置窗口
                    win.clear(); matched = 0;
                    left = right;
                }
            }
