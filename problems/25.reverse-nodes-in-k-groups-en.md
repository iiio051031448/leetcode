...
![reverse linked list](https://p.ipic.vip/ajewar.jpg)
...
![reverse linked list range in (start, end)](https://p.ipic.vip/4khz8w.jpg)


For example(as below pic)，`head=[1,2,3,4,5,6,7,8], k = 3`


![reverse k nodes in linked list](https://p.ipic.vip/o04jk9.jpg)
...
public:
    ...
    ListNode* reverseKGroup(ListNode* head, int k) {
        ...
        ListNode *breakL = dummy;  // 左断点
        ListNode *breakR = nullptr; // 右断点

        if (k == 1) return head;

        while(head) {   // head 一直为 group 的 head
            // rTail 为 group 的 tail
            // 通过循环找 rTail, 并查看剩余部分长度是否大于等于 k
            ListNode *rTail = breakL;
            for (int i = 0; i < k; i++) {
                rTail = rTail->next;
                if (!rTail) {
                    return dummy->next;
            ...
            breakR = rTail->next; // 记录 右断点

            tie(head, rTail) = reverseSub(head, rTail);  // 反转后 head 和 rTail 互换
