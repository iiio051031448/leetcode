...
### LYJ:DFS 和 回溯法，是同一个东西吗
回溯法 是一种算法范式，本质是：做选择 → 递归 → 撤销选择。它通常用 DFS 作为遍历手段，
但核心是在每一步尝试所有可能，遇到死路就回退（回溯）。

可以用这个关系理解：**回溯法 = DFS + 剪枝 + 状态重置**

所以回溯法依赖 DFS 来遍历解空间树，但多了"剪枝"和"撤销选择"这两个关键步骤。常见写法里那个 path.pop() 就是回溯特有的"撤销"动作——DFS 本身不需要这个。
