...
public:
    void merge(vector<int>& nums1, int m, vector<int>& nums2, int n) {
        int current = m + n - 1; // 注意这里时 -1 !!!
        while (current >= 0) {
            if (n == 0) break;
            if (m < 1) {
                while (n > 0) nums1[current--] = nums2[--n]; // 这里优化为 while 直接 copy nums2, 因为 num1 移动完了。
                break;
            }

            if (nums1[m - 1] > nums2[n - 1]) { // 注意这里 -1 的下标
                nums1[current--] = nums1[--m];
            } else {
                nums1[current--] = nums2[--n];
            }
